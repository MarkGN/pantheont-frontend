...
function PlantSearch () {
  ...
  return <div>
    ...
    <div className="row card-holder">
      ...
      {plants.filter(filterAllTags).sort(sort).map((data, ix) => <Plant key={ix} colors={data.colors} description={data.description} effect={data.effect} name={data.name} reagentType={data.reagentType} tags={(data.tags && data.tags.join(", ")) || ""} />)}
    </div>
  </div>
}
